...
class Logger {
  ...
  constructor (name, opts = {}) {
    ...
    // and external.
    if (env.DEBUG) {
      const debugSettings = split(env.DEBUG)
    ...
    Object.defineProperty(this, 'logLevel', {
      ...
      set (value) {
        ...
        if (Array.isArray(value)) {
          let external = []
          if (env.DEBUG) {
            external = split(env.DEBUG).filter(s => s && !s.startsWith(this.prefix))
          }
          // remember our current settings after removing empties
          this.current = [...new Set(value)].filter(s => s)
    ...
    if (internal.length) {
      this.addEnabled(internal.map(s => s.slice(this.prefix.length)))
    }
  }
  ...
  addEnabled (levels) {
    ...
    return this.logLevel = this.current.concat(levels.filter(s => this.current.indexOf(s) < 0))
  }
