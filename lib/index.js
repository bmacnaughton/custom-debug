...
class Logger {
  ...
  constructor(name, opts = {}) {
    ...
    // if set this takes precedence over DEBUG so replace any internal already found.
    if (envName in env) {
   ...
   *
   * @example
   * instance.addEnabled('warn,debug')
   */
  addEnabled(levels) {
    ...
    return this.logLevel = this.current.concat(levels.filter(s => !this.current.includes(s)));
  }

  /**
   * Remove log levels from the current set.
   *
   * @method instance.removeEnabled
   ...
   *                              error.
   * @example
   * var previousLogLevel = instance.logLevel
   * instance.addEnabled('debug')
   * instance.removeEnabled(previousLogLevel)
   */
  removeEnabled(levels) {
    ...
    this.logLevel = this.current.filter(s => !levels.includes(s));

    return this.current.join(',');
  }

  has(level) {
    return this.current.includes(level);
  }
